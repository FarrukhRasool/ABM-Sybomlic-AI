...
class TouristAgent(BaseAgent):
    ...
    def __init__(self, model, start_pos: tuple):
        ...
        self.carrying_waste  = False      # Internal waste-carrying state
        self._last_pos       = None       # Used to reduce immediate backtracking during movement
        self._steps_taken    = 0          # Visit duration counter                         
        self._max_waste      = random.randint(
            self.MAX_WASTE_MIN, self.MAX_WASTE_MAX  # Personal waste-production limit for this visit
        )
